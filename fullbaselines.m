...
function [fullspatialavg_germs,fullspatialavg_compeff] = fullbaselines(dim,initcond,numls,maxsp,dist_a,dist_p,prop,hab,beta,alpha,dmech,ddstrength,models,rootdir,breakpoints,reprohab)   
    
if hab == 1;
propBsuitable = zeros(numls,maxsp);
propRsuitable = zeros(numls,1);
end

rando = 0.1563;
rng(rando)

%fullspatialavg_seeds = zeros(numls,maxsp); %AVERAGED OVER INITIAL CONDITIONS FOR A LANDSCAPE
fullspatialavg_germs = zeros(numls,maxsp);
fullspatialavg_compeff = zeros(numls,maxsp);
for t = 1:numls
...
filepathString = char(['/Users/s_catella/Documents/MATLAB/simulation2019Jan/equivalent/' dmech '/' ddstrength '/' char(models(1))]);
cd(filepathString)
...
if hab == 1
     ...
     propRsuitable(t) = length(find(A>=reprohab(1,1) & A<=reprohab(1,2)))/dim^2;
...
for n = 1:maxsp
    ...
    if hab == 1
       germs{j,n} = mean2(germinating)/propRsuitable(t,1);
    else
    ...
    for x = 1:dim
        for y = 1:dim
            ...
            Rbysps = zeros(1,length(allwhichsps)); %need the competitive effect of all species,
                ...
                for n = 1:4
                if hab == 1
                compeff{j,n} = mean2(comps(:,:,n))/propRsuitable(t,1);
                else
...
fullspatialavg_germs(t,n) = mean(cat(1,germs{:,n}));
fullspatialavg_compeff(t,n) = mean(cat(1,compeff{:,n}));
end
end
