...
if strcmp(reproduction(repro),'neutral') == 1
    models = {'neutRdiffG#diffC'};
else
    models = {'simRdiffG#diffC'};
end
...
C = A; %for the uncorrelated landscapes, do competition according to A because then you can
       %still use landscape B for germination which is what defines
       %"habitat", and it makes sense in this context for habitat to
       %consist of places where you can germinate.
...
cd(filepathString)
if saveit == 1
        filename = char(['dim_' num2str(dim) '_' num2str(t) '_' char(models(mods)) '_' dmech ddstrength '.mat']);
        save(filename);
end
cd(rootdir);
